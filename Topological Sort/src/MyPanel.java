...
public class MyPanel extends JPanel {
    ...
    public MyPanel()
    {
        ...
        bottomPanel.add(topologicalSortButton);
    ...
    // SORTAREA TOPOLOGICA EXEMPLUL 1
    public void topologicalSort() {
        int numNodes = listaNoduri.size();
        int[] inDegree = new int[numNodes]; // Vector de grade de intrare
        ...
        // Coada pentru nodurile cu gradul de intrare 0
        ...
        // Lista pentru rezultatul sortării topologice
        ...
        while (!queue.isEmpty()) {
            ...
            for (Arc arc : listaArce) {
                Node startNode = getNodeAtP(arc.getStartPoint());
                Node endNode = getNodeAtP(arc.getEndPoint());
                if (startNode != null && endNode != null && startNode == currentNode) {
                    int endIndex = listaNoduri.indexOf(endNode);
                    inDegree[endIndex]--;
        ...
        // Verificăm dacă a fost posibilă sortarea (adică dacă graful era aciclic)
        if (topologicalOrder.size() != numNodes) {
            System.out.println("Graful are cicluri și nu poate fi sortat topologic.");
        } else {
            System.out.println("Sortarea topologică:");
    ...
    private Integer getNumber(Node n)
    {
        for(Node node : listaNoduri)
    ...
    private int source(){
        ...
        for (int i = 0; i < numNodes; i++) {
            if (inDegree[i] == 0) {
                return listaNoduri.get(i).getNumber() - 1;
            }
        }
        return -1;
    }
...
//        System.out.println("LISTA NODURILOR ESTE: " + listaNoduri);
        int S = source();
//        System.out.println("S: " +  S);
        ...
        while(!V.isEmpty()){
            ...
            boolean exist = false;
            for(Arc arc : listaArce) {
                if (arc.getStartNode().getCoordX() == x.getCoordX() && arc.getStartNode().getCoordY() == x.getCoordY() && verifyYinU(arc, U) && exist == false) {
                    U.remove(Uindex(U, getNumber(arc.getEndNode())));
                    V.add(arc.getEndNode(getNumber(arc.getEndNode())));
                    t = t + 1;
                    t1.set(getNumber(arc.getEndNode())-1, t);
                    p.set(getNumber(arc.getEndNode())-1, x.getNumber());
                    exist = true;
                }
            }
            if(!exist) {
        ...
        if(verify2()){
            ...
            for(int i=0; i<t2.size(); i++)
                pairs.add(new int[]{t2.get(i), i + 1});
            pairs.sort((a, b) -> Integer.compare(b[0], a[0]));
            System.out.println("Sortarea topologica: ");
            for (int[] pair : pairs) {
